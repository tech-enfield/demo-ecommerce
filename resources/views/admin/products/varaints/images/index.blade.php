...
@section('title', 'Product Variant Image | Store 9Nepal Admin Dashboard')
    ...
    <div class="grid grid-cols-1 gap-6 lg:grid-cols-2">
        ...
        <aside class="lg:col-span-1">
            <div class="rounded-2xl border border-gray-200 bg-white p-6 dark:border-gray-800 dark:bg-white/5">
                ...
                <form action="{{ route('admin.products.variants.images.store') }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    ...
                    <div class="space-y-5">
                        ...
                        <div class="mb-4">
                            <label class="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-400">
                                Images <span class="text-error-500">*</span> <span class="text-gray-500">(Please upload on
                                    sorting order)</span>
                            </label>
        ...
        dark:border-gray-700 dark:bg-dark-900">
            ...
            hover:text-gray-900 dark:hover:text-white transition">
                                ...
                                <span id="file-count" class="text-sm text-gray-600 dark:text-gray-400">
                                    No files chosen
                                </span>
                            </div>
                        ...
                        <div>
                            <label class="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-400">
                                Alt Texts <span class="text-error-500">*</span> <span
                                    class="text-gray-500">(semi-colon-separated <strong>;</strong> with image order)</span>
                            </label>
                            <input type="text" name="alt_text"
                                placeholder="front view; keyboard and RGB lighting; side ports; back cooling vents"
                                value="{{ old('alt_text') }}"
        ...
        <aside class="lg:col-span-1">
            <div
                ...
                <div class="grid grid-cols-1 gap-4 mb-4 sm:grid-cols-3 sm:items-center">
                    ...
                    <!-- Right: Search Form -->
                    <form action="{{ route('admin.products.images.index') }}" method="GET"
                                        ...
                                        <td class="py-3">
                                            <div class="flex items-center">
                                                <div class="text-gray-500 text-theme-sm dark:text-gray-400 flex gap-2">
                                                    {{-- <a href="{{ route('admin.products.images.show', $item->id) }}">
                                                    ...
                                                    <a href="{{ route('admin.products.images.edit', $item->id) }}">
                                                        <button title="edit {{$item->productVariant->product->name }} - {{ $item->alt }}">
                                                    ...
                                                    <form action="{{ route('admin.products.images.destroy', $item->id) }}"
                                                        method="POST">
                                                        @csrf @method('DELETE')
                                                        <button title="delete {{$item->productVariant->product->name }} - {{ $item->alt }}"
                                                            onclick="return confirm('Are you sure you want to delete {{$item->productVariant->product->name }} - {{ $item->alt }} product image?')">
            ...
            if (input.files && input.files.length > 0) {
                ...
                Array.from(input.files).forEach((file, index) => {
                    ...
                    reader.onload = function(e) {
                        ...
                        img.className =
                            "w-full max-h-40 object-contain rounded-lg shadow-sm p-1";

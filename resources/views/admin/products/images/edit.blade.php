...
@section('title', 'Product Image | Store 9Nepal Admin Dashboard')
    ...
    <div class="grid grid-cols-1 gap-6 lg:grid-cols-2">
        ...
        <aside class="lg:col-span-1">
            <div class="rounded-2xl border border-gray-200 bg-white p-6 dark:border-gray-800 dark:bg-white/5">
                ...
                <form action="{{ route('admin.products.images.update', $product_image->id) }}" method="POST"
                    ...
                    <div class="space-y-5">
                        ...
                        <div>
                            <label class="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-400">
                                Product Name <span class="text-error-500">*</span>
                            </label>
                            <input type="text" name="name" value="{{ old('name') ?? $product_image->product->name }}"
                                readonly
                        ...
                        <div>
                            ...
                            <div
                                ...
                                <span id="file-name" class="text-sm text-gray-600 dark:text-gray-400">
                                    {{ old('image') ? basename(old('image')) : asset('storage/' . $product_image->path) }}
                                </span>
                            </div>

                            <!-- Preview -->
                            <div class="mt-3 flex relative max-w-full">
                                <img id="image-preview"
                                    src="{{ old('image') ? asset('storage/' . old('image')) : asset('storage/' . $product_image->path) }}"
                                    class="max-h-40 rounded-lg shadow-md object-contain" />
                                <button type="button" id="remove-image" onclick="removeImage()"
                                    class="absolute top-1 right-1 bg-red-500 text-white rounded-full w-5 h-5 flex items-center
                        ...
                        <div>
                            <label class="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-400">
                                Alt Text <span class="text-error-500">*</span>
                            </label>
                            <input type="text" name="alt_text" placeholder="Front view"
                                value="{{ old('alt_text') ?? $product_image->alt }}"
                                class="dark:bg-dark-900 h-11 w-full rounded-lg border border-gray-300 bg-transparent px-4 py-2.5
                                text-sm text-gray-800 shadow-theme-xs placeholder:text-gray-400 focus:border-brand-300
                                focus:outline-hidden focus:ring-3 focus:ring-brand-500/10
                                dark:border-gray-700 dark:bg-gray-900 dark:text-white/90 dark:placeholder:text-white/30
                                            ...
                                            <div class="flex items-center">
                                                <div class="text-gray-500 text-theme-sm dark:text-gray-400 flex gap-2">
                                                    <a href="{{ route('admin.products.images.edit', $item->id) }}">
                                                        <button
                                                            title="edit {{ $item->product->name }} - {{ $item->alt }}">
                                                    ...
                                                    <form action="{{ route('admin.products.images.destroy', $item->id) }}"
                                                        ...
                                                        @csrf @method('DELETE')
                                                        <button
                                                            title="delete {{ $item->product->name }} - {{ $item->alt }}"
                                                            onclick="return confirm('Are you sure you want to delete {{ $item->product->name }} - {{ $item->alt }} product image?')">

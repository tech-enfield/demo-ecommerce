...
@section('title', 'Product Variant Attribute | Store 9Nepal Admin Dashboard')
        ...
        <aside class="lg:col-span-1">
            <div class="rounded-2xl border border-gray-200 bg-white p-6 dark:border-gray-800 dark:bg-white/5">
                ...
                <h3 class="text-lg font-semibold text-gray-800 dark:text-white/90 mb-4">
                    Assign Attributes to Product Variant
                </h3>
                    ...
                    <div class="space-y-5">
                        ...
                        <div>
                            ...
                            <select name="variant_id" id="variantSelect"
                                ...
                                <option value="" selected disabled>Select a product variant</option>
                                @foreach ($variants as $item)
                                    <option value="{{ old('variant_id') ?? $item->id }}"
                        ...
                        <div>
                            <label class="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-400">
                                Attribute Value <span class="text-error-500">*</span>
                            </label>
                            <select name="attribute_value_id[]" id="attributeSelect" multiple
                                class="w-full rounded-lg border border-gray-300 dark:border-gray-700 dark:bg-gray-900">
                                @foreach ($attributes as $item)
                                    <option value="{{ old('attribute_value_id[]') ?? $item->id }}"
                                        class="bg-white text-gray-700 dark:bg-gray-900 dark:text-gray-300">
                                        {{ $item->value }}
                                    </option>
                                @endforeach
                            </select>
                            <x-input-error :messages="$errors->get('attribute_value_id[]')" class="mt-2" />
                        </div>
                    </div>
        ...
        <aside class="lg:col-span-1">
            <div
                ...
                <div class="grid grid-cols-1 gap-4 mb-4 sm:grid-cols-3 sm:items-center">
                    ...
                    <!-- Right: Search Form -->
                    <form action="{{ route('admin.products.variants.attributes.index') }}" method="GET"
                        class="col-span-1 sm:col-span-2 flex gap-2">
                        <input type="text" name="product_variant_sku" placeholder="Search by product variant sku."
                            value="{{ old('product_variant_sku') ?? request('product_variant_sku') }}"
                ...
                <div class="w-full overflow-x-auto">
                    <table class="min-w-full">
                                        ...
                                        <td class="py-3">
                                            <div class="flex items-center">
                                                <div class="text-gray-500 text-theme-sm dark:text-gray-400 flex gap-2">
                                                    <a
                                                        href="{{ route('admin.products.variants.attributes.show', $item->id) }}">
                     ...
                     C19.4,32.6,22.4,32.6,23.4,32.4z M11.1,20.7c0-5.2,4.4-9.4,9.9-9.4s9.9,4.2,9.9,9.4S26.5,30,21,30S11.1,25.8,11.1,20.7z" />
                                                    ...
                                                    <a
                                                        href="{{ route('admin.products.variants.attributes.edit', $item->id) }}">
                                                    ...
                                                    <form
                                                        action="{{ route('admin.products.variants.attributes.destroy', $item->id) }}"
                                                        method="POST">
                                                        @csrf @method('DELETE')

        ...
        <aside class="lg:col-span-1">
            <div class="rounded-2xl border border-gray-200 bg-white p-6 dark:border-gray-800 dark:bg-white/5">
                ...
                <h3 class="text-lg font-semibold text-gray-800 dark:text-white/90 mb-4">
                    Assign Color Family to Product Variant
                </h3>

                <form action="{{ route('admin.products.variants.assign.color.family.store') }}" method="POST">
                    @csrf
                    ...
                    <div class="space-y-5">
                        ...
                        <div>
                            ...
                            <input type="text" name="product_variant_id" value="{{ $variant->id }}" hidden>
                            <input type="text" value="{{ $variant->product->name . ' - ' . $variant->title }}" readonly
                       ...
                       dark:focus:border-brand-800" />
                            <x-input-error :messages="$errors->get('product_variant_id')" class="mt-2" />
                        </div>
            ...
            <div
                ...
                <div class="col-span-1">
                    <h3 class="text-lg font-semibold text-gray-800 dark:text-white/90">
                        Product Variant & Color Family Relation
                    </h3>
                </div>
                                            ...
                                            <div class="flex items-center">
                                                <div class="text-gray-500 text-theme-sm dark:text-gray-400 flex gap-2">
                                                    ...
                                                    {{-- <a href="{{ route('admin.products.images.edit', $item->id) }}">
                                                        <button
                                                            title="edit {{ $item->productVariant->product->name }} - {{ $item->alt }}">
                                                    ...
                                                    <form action="{{ route('admin.products.variants.assign.color.family.delete', $item->id) }}"
                                                        method="POST">
                                                        @csrf @method('DELETE')
                                                        <button
                                                            title="delete {{ $item->productVariant->product->name }} - {{ $item->alt }}"

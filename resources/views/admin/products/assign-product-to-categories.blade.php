...
@section('title', 'Assign Product To Categories')
    ...
    <div class="grid grid-cols-1 gap-6 lg:grid-cols-2">
        ...
        <aside class="lg:col-span-1">
            <div class="rounded-2xl border border-gray-200 bg-white p-6 dark:border-gray-800 dark:bg-white/5">
                ...
                <form action="{{ route('admin.products.assign.category.store') }}" method="POST">
                    @csrf
                    ...
                    <div class="space-y-5">
                        ...
                        <div>
                            <label class="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-400">
                                Product <span class="text-error-500">*</span> <span></span>
                            </label>
                        ...
                        <div>
                            <label class="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-400">
                                Assign Categores <span class="text-error-500">*</span>
                            </label>
        ...
        <aside class="lg:col-span-1">
            <div
                ...
                <div class="grid grid-cols-1 gap-4 mb-4 sm:grid-cols-2 sm:items-center">
                    <!-- Left: Title -->
                    ...
                    <!-- Right: Search Form -->
                    <form action="{{ route('admin.products.assign.category') }}" method="GET"
                        class="col-span-1 sm:col-span-1 flex gap-2">
                        <input type="text" name="category_name" placeholder="Search by categories name."
                            value="{{ old('category_name') ?? request('category_name') }}"
                                            ...
                                            <div class="flex items-center">
                                                <div class="text-gray-500 text-theme-sm dark:text-gray-400 flex gap-2">
                                                    <form action="{{ route('admin.categories.delete.relation', $item->id) }}"
                                                        method="POST">
                                                        @csrf @method('DELETE')
                                                        <button title="delete {{ $item->product->name }} - {{ $item->category->name }}"
                                                            onclick="return confirm('Are you sure you want to delete {{ $item->product->name }} - {{ $item->category->name }} relation?')">
